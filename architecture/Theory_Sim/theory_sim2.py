...
Layer 2: Composition/Unit Choice Probability
...
Layer 1 (carried forward):
  Capture, MG, EA, singularity detection, DAG instrumentation.
  All Layer 1 mechanics unchanged.

Layer 2 addition:
  COMP_CHOICE_PROB controls the split between compositions and base symbols
  when sampling a token for point mutations and insertions.

    COMP_CHOICE_PROB = 0.0 -> always sample a base symbol (Layer 1 behavior
                              when no compositions exist; pure base drift)
    COMP_CHOICE_PROB = 1.0 -> always sample a composition (if any exist),
                              never inject base symbols via mutation

  When a composition is chosen, selection is uniform random over all current
  composition IDs in the MG. When a base symbol is chosen, selection is
  uniform random over {ENC_BASE0, ENC_BASE1}.

  If COMP_CHOICE_PROB > 0 but no compositions exist yet, falls back to base
  symbol sampling automatically.

  Theoretical prediction:
    - Intermediate values extend the singularity horizon relative to Layer 1
      (compositions spread more slowly because base injection is reduced and
      the dominant comp faces more competition from other comps via mutation)
    - High values (-> 1.0) deprive the population of base symbol material,
      disrupting the boundary geometry dynamics Layer 0 established and
      reverting toward Layer 0 behavior (no stable delimiter substrate)
    - The singularity is not prevented at any value; choice only modulates
      the timescale and the degradation of Layer 0 geometry en route

  COMP_CHOICE_PROB is stamped on every plot title and output folder name so
  individual runs can be compared visually.

Outputs under ./theory_sim_2_ccp{COMP_CHOICE_PROB}/:
...
MUTATION_PROB           = 0.10   # depth-0 tokens
DELIMITED_MUTATION_PROB = 0.5   # interior tokens

PROGRESS_EVERY = 20

# Singularity threshold: fraction of all non-delimiter tokens held by the
# single most frequent composition ID.
SINGULARITY_THRESHOLD = .950

# -----------------------------------------------------------------------
# Layer 2 parameter
# COMP_CHOICE_PROB: probability of sampling a composition ID (vs base symbol)
# when selecting a token for point mutation or insertion.
#   0.0 = always base symbol  (approaches Layer 1 neutral drift behavior)
#   1.0 = always composition  (starves base pool, disrupts Layer 0 geometry)
# Composition selection is uniform random over all current MG IDs.
# Falls back to base symbol if no compositions exist yet.
# -----------------------------------------------------------------------
COMP_CHOICE_PROB = 0.30   
...
# Output folder stamped with the parameter value
OUTPUT_FOLDER = f"theory_sim_2_ccp{COMP_CHOICE_PROB:.2f}"
...
def sample_token(rng, mg):
    """
    Layer 2 token sampler for point mutations and insertions.

    With probability COMP_CHOICE_PROB, sample uniformly from all current
    composition IDs. With probability (1 - COMP_CHOICE_PROB), sample a
    base symbol uniformly from {ENC_BASE0, ENC_BASE1}.
...
def mutate_org(org, mg, rng):
    """
    Identical to Layer 1 except point mutations and insertions call
    sample_token() instead of random_non_delimiter(), applying the
    COMP_CHOICE_PROB split.
    """
    i      = 0
    inside = False
    ...
    while i < len(org):
        ...
        if rng.random() < rate:
            ...
            if op == "point":
                org[i] = sample_token(rng, mg)   # Layer 2: choice-controlled
                i += 1
                continue

            if op == "insertion":
                new_tok = sample_token(rng, mg)   # Layer 2: choice-controlled
                if rng.random() < 0.5:
...
def run():
    ...
    # Plot label stamped on every title
    param_label = f"ccp={COMP_CHOICE_PROB:.2f}"
        ...
        for gen in range(GENERATIONS + 1):
            ...
            if gen % PROGRESS_EVERY == 0:
                ...
                print(f"  gen {gen:4d}  mg={mg_size:5d}  "
                      ...
                      f"dom={dom_frac:.3f}  dag_roots={dag_r:4d}  "
                      f"dag_depth={dag_md:3d}  A0={A0:6d}  A2={A2:5d}  "
    ...
    ax.set_title(title("Boundary Geometry Dynamics (Layer 2)"))
    ...
    fig.suptitle(title("Segment Length Dynamics (Layer 2)")); fig.tight_layout()
    ...
    axes[0, 0].set_title(title("Gap State Distribution"))
    axes[0, 0].set_xlabel("Generation"); axes[0, 0].set_ylabel("Fraction of gaps")
    axes[0, 0].legend(fontsize=8)

    axes[0, 1].plot(gens, fgap0_list, color="tab:red"); mark_singularity(axes[0, 1])
    axes[0, 1].set_title(title("Contact Rate (gap=0)"))
    ...
    axes[0, 2].set_title(title("Expansion-Ready Rate (gap=1)"))
    ...
    axes[1, 0].set_title(title("Contact Flux (|Δ gap=0| per generation)"))
    ...
    axes[1, 1].set_title(title("Mean Gap vs Contact Rate"))
    ...
    axes[1, 2].set_title(title("Segment Length Variance"))
    axes[1, 2].set_xlabel("Generation"); axes[1, 2].set_ylabel("Variance")

    fig.suptitle(title("Contact Dynamics Suite (Layer 2)"), fontsize=14); fig.tight_layout()
    ...
    ax.set_title(title("Composition Library Growth (Layer 2)")); fig.tight_layout()
    ...
    mark_singularity(ax)
    ax.set_xlabel("Generation"); ax.set_ylabel("Token count (population total)")
    ax.set_title(title("Token Type Distribution (Layer 2)")); ax.legend(); fig.tight_layout()
    fig.savefig(os.path.join(OUTPUT_FOLDER, "token_counts.png"), dpi=150)
    plt.close(fig)

    # -- Layer 1: Base vs composition fraction + dominant comp --
    fig, ax = plt.subplots(figsize=(10, 4))
    ...
    ax.set_title(title("Base vs Composition Fractions + Dominant Composition (Layer 2)"))
    ...
    ax.set_title(title("Total Alphabet Size Over Time (Layer 2)")); fig.tight_layout()
    ...
    axes[0, 0].set_title(title("DAG Nodes (= MG size)"))
    ...
    axes[0, 1].set_title(title("DAG Edges (composition references)"))
    ...
    axes[0, 2].set_title(title("DAG Depth (compositional hierarchy)"))
    ...
    axes[1, 0].set_title(title("DAG Roots (top-level compositions)"))
    ...
    axes[1, 1].set_title(title("Largest Connected Component"))
    ...
    axes[1, 2].set_title(title("Edge Density (edges / nodes)"))
    axes[1, 2].set_xlabel("Generation"); axes[1, 2].set_ylabel("Avg out-degree")

    fig.suptitle(title("DAG Dynamics — Compositional Hierarchy (Layer 2)"), fontsize=14)
    fig.tight_layout()
